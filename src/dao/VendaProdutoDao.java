...
public class VendaProdutoDao {
    ...
    public void inserir(VendaProduto vendaProduto) {
        try {
            ...
            ps.setDouble(4, vendaProduto.getProduto().getValorUnidade());
    ...
    public void excluir(VendaProduto vendaProduto){
        try {
            String SQL = "DELETE FROM venda_produto where produto_id = ?";
            
            ps = conexao.getConn().prepareStatement(SQL);
            
            ps.setInt(1, vendaProduto.getProduto().getProdutoId());           
    ...
    public void editar(VendaProduto vendaProduto) {
        try {
            ...
            ps.setInt(1, vendaProduto.getQuantidade());
            ps.setDouble(2, vendaProduto.getValorTotalItem());
            ps.setInt(3, vendaProduto.getId()); // Usa o ID do item para atualizar
            ps.executeUpdate();
            ps.close();
        } catch (SQLException ex) {
            ex.printStackTrace();
